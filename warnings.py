# Embedded file name: /home/boris/undernetbot/build/run/out00-PYZ.pyz/warnings
...
def warn_explicit(message, category, filename, lineno, module = None, registry = None, module_globals = None):
    ...
    if module is None:
        ...
        if registry is None:
            registry = {}
        text = isinstance(message, Warning) and str(message)
        category = message.__class__
    else:
        text = message
    ...
    if registry.get(key):
        ...
        if fxn_code:
            ...
            if 'line' not in args and not fxn_code.co_flags & CO_VARARGS:
                showwarning_msg = "functions overriding warnings.showwarning() must support the 'line' argument"
