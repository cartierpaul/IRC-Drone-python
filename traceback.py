# Embedded file name: /home/boris/undernetbot/build/run/out00-PYZ.pyz/traceback
        ...
        try:
            ...
            if not filename:
                ...
                if badline is not None:
                    lines.append('    %s\n' % badline.strip())
                    caretspace = offset is not None and badline.rstrip('\n')[:offset].lstrip()
                    caretspace = (' ' for c in caretspace if not (c.isspace() and c))
                    lines.append('   %s^\n' % ''.join(caretspace))
                value = msg
