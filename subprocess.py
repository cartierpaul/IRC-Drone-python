# Embedded file name: /home/boris/undernetbot/build/run/out00-PYZ.pyz/subprocess
    ...
    if mswindows:
            ...
            if self.returncode is not None:
                ...
                if timeout is not None:
                    ...
                    while True:
                        ...
                        if not (pid == self.pid or pid == 0):
                            raise AssertionError
                            if pid == self.pid:
                                self._handle_exitstatus(sts)
                                break
                            remaining = endtime - time.time()
                            raise remaining <= 0 and TimeoutExpired
        ...
        def _communicate_with_select(self, input, endtime):
            ...
            while self._read_set or self._write_set:
                ...
                try:
                    rlist, wlist, xlist = select.select(self._read_set, self._write_set, [], remaining)
                except select.error as e:
                    if e.args[0] == errno.EINTR:
                        continue
                    raise
                ...
                if not rlist:
                    if not wlist:
                        ...
                        if self.stdout in rlist:
                            ...
                            stdout.append(data)
                        data = self.stderr in rlist and os.read(self.stderr.fileno(), 1024)
